...
class PasswordManager():
    ...
    def save_master_password(self, eb1, eb2):
        ...
        if password1 == password2:
            ...
            insert_command = "INSERT INTO login VALUES(1,\""+ hashed_password + "\")"
    ...
    def check_master_password(self, eb):
        ...
        self.cursor.execute( "SELECT * FROM login WHERE user_id = 1 AND password = \""+hashed_password+"\"")
    ...
    def password_vault_screen(self):
            ...
            add_password_btn.grid(row=1, column=2, pady=35)
            ...
            if self.cursor.fetchall():
                ...
                while True:
                    ...
                    # print(array)
                    platform_label = Label(second_frame, text=(array[i][0]))
                    ...
                    update_btn = Button(second_frame, text="Update Password",
                                        command=partial(vault_methods.update_password, array[i][1], self.password_vault_screen))
                    update_btn.grid(column=4, row=i + 3, pady=10, padx=10)
                    remove_btn = Button(second_frame, text="Delete Password",
                                        command=partial(vault_methods.remove_password, array[i][1], self.password_vault_screen))
